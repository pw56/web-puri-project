...
- 属性: `button-1`
- 表示:
> アクセントカラーの背景、メインカラーの文字のボタン。
> ボタン押下時に背景が、0.5秒かけてアクセントカラーから薄い色に変化するアニメーションを表示。  
> ![button-1](button-1.png)

## ボタン2
- 属性: `button-2`
- 表示:
> メインカラーの背景、アクセントカラーの文字、アクセントカラーの縁のボタン。
> CSSの`box-sizing`プロパティと`border-box`を使用して、ボタンの縁を内側に収まるようにする。
> ボタン押下時に文字、縁がそれぞれ、0.5秒かけて文字がアクセントカラーから薄い色に変化するアニメーションを表示。  
