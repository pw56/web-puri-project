...
> 顔の全てのランドマークの検出後に別のWeb Workerを立ち上げ、パーツごとの周囲との色の変化(周囲との数値の比較で突出した部分)や座標分布の変化予測(微分積分など)などから境界線を微細化し、座標情報の精度を上げる。
> 虹彩の部分はモデルのランドマークだけでは精度が不十分なので、周囲の白目の部分との色の変化などをアルゴリズムを使用し、高速に検出する。
