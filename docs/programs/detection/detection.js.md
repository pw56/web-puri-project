...
## detectFaces (関数)
- 概要
> 入力された画像をU²-Netモデルで背景削除し、ImageDataオブジェクトに変換。
...
> 抽出処理にはTensorFlow.jsのCOCO-SSD、Face Landmarks DetectionモデルとMediaPipe Face Meshモジュールを使用する。
...
> > 人物の抽出は、インスタンス生成時にコンストラクタに渡された背景削除済みの画像から、"#selfieSegmentation()"で人物の縁を配列に変換する。
