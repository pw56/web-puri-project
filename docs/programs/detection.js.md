...
## detectFaces (関数)
- 概要
> 入力された画像からTensorFlow.jsのCOCO-SSDモデルを用いて単数または複数の人物を検出しランドマークの処理範囲を限定し、検出した人物の信頼度(score)が0.5以上のものだけ、それぞれの人物の顔でFaceクラスのインスタンスを生成し、それらのインスタンスを配列で返す。
...
> > 人物の抽出は、MediaPipe Selfie Segmentationモデルで検出された人物のマスク画像から、"#selfieSegmentation()"で人物の縁を配列に変換する。
> > 人物の抽出の際に、ランドマーク検出で検出された座標情報とマスク画像を比較し、両者の平均などを利用して座標の精度を上げる。
...
> > ```
>
> ### selfieSegmentation (プライベートメソッド)
- 概要
> WegGLを用いて("selfie-segmentation.glsl"を使用)入力された画像を、ピクセルごとのアルファ値から、抽出された人物と削除された背景の境界線を高速に配列に抽出して、クラスのプライベート変数に保存する。
